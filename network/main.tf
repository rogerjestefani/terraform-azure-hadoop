...
resource "azurerm_route_table" "hadoop" {
  name                = "${local.name}-rt"
  location            = "${local.location}"
...
resource "azurerm_network_security_group" "hadoop" {
  name                = "${local.name}-nsg"
  location            = "${local.location}"
  resource_group_name = "${azurerm_resource_group.hadoop.name}"
}

resource "azurerm_subnet" "hadoop" {
  name                      = "${local.name}"
  resource_group_name       = "${local.vnet_resource_group}"
  virtual_network_name      = "${local.vnet_name}"
  address_prefix            = "${local.subnet}"
  route_table_id            = "${azurerm_route_table.hadoop.id}"
  network_security_group_id = "${azurerm_network_security_group.hadoop.id}"
}
